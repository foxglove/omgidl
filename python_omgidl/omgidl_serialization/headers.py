...
def read_delimiter_header(
    view: memoryview, offset: int, fmt_prefix: str
) -> Tuple[int, int]:
...
def write_delimiter_header(
    buffer: bytearray, offset: int, length: int, fmt_prefix: str
) -> int:
...
def write_member_header(
    buffer: bytearray, offset: int, field_id: int, size: int, fmt_prefix: str
) -> int:
